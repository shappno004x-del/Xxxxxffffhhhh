...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'shappno_premium_vps_secret_key_123!@#')

# ============================================
# সেলফ-পিং (উন্নত ভার্সন)
# ============================================
def start_self_pinger():
    # Render-এর ডিফল্ট URL, SELF_URL অথবা লোকালহোস্ট সেট করা
    port = os.environ.get('PORT', 8080)
    external_url = os.environ.get("RENDER_EXTERNAL_URL") or os.environ.get("SELF_URL")
    
    if external_url:
        # /ping লিংকে রিকোয়েস্ট পাঠাবে
        ping_url = f"{external_url}/ping"
    else:
        # কোনো লিংক না পেলে লোকালহোস্টে কাজ করবে
        ping_url = f"http://127.0.0.1:{port}/ping"
        print("⚠️ External URL not set. Falling back to localhost self-ping.")

    def _pinger():
        # সার্ভার পুরোপুরি চালু হওয়ার জন্য একটু সময় দেওয়া
        time.sleep(10)
        while True:
            try:
                response = requests.get(ping_url, timeout=10)
                print(f"✅ Self-ping sent to {ping_url} - Status: {response.status_code}")
            except Exception as e:
                print(f"❌ Self-ping failed: {e}")
            time.sleep(240)  # ✅ ৪ মিনিট (২৪০ সেকেন্ড) পরপর পিং

    thread = threading.Thread(target=_pinger, daemon=True)
    thread.start()
    print("✅ Self-pinger thread started")
...
DEFAULT_CONFIG = {
    "site_title": "SHAPPNO PREMIUM VPS",
    "site_header": "SHAPPNO VPS",
    "theme": "matrix",
    "font_family": "default",
    "admin_password": hashlib.sha256("shappno".encode()).hexdigest(),
    "colors": {
        "matrix": {
            "name": "Matrix Green",
            "primary": "#00ff00",
    ...
    "passwords": {
        "secret": hashlib.sha256("shappno".encode()).hexdigest(),
        "user": hashlib.sha256("shappno".encode()).hexdigest()
    }
}
    ...
    try:
        for line in iter(proc_obj.stdout.readline, ''):
            ...
            if line:
                ...
                if cleaned_line:
                    MAX_LOG_LINES = 500
                    if len(SERVERS[server_id]['logs']) > MAX_LOG_LINES:
                        SERVERS[server_id]['logs'] = SERVERS[server_id]['logs'][-MAX_LOG_LINES:]
...
def start_server_internal(server_id, server):
    ...
    try:
        ...
        # Windows এবং Linux-এর জন্য আলাদা
        if os.name == 'nt':
...
def auto_restarter():
    while True:
        ...
        try:
            for server_id, server in list(SERVERS.items()):
                try:
                    if server.get('status') == 'running' and server.get('auto_restart'):
                        ...
                        last_start = server.get('last_start_time', time.time())
                        
                        if time.time() - last_start >= interval_sec:
            ...
            print(f"Auto-restarter main loop error: {e}")
    ...
    try:
        if request.method == 'POST':
            ...
            if hashed_input == CONFIG['passwords']['secret']:
                ...
                return render_template('login.html', error="Invalid password", config=CONFIG, colors=current_colors)
        
        current_colors = CONFIG['colors'].get(CONFIG['theme'], CONFIG['colors']['matrix'])
        return render_template('login.html', config=CONFIG, colors=current_colors)
    except Exception as e:
...
def rename_file(server_id):
    try:
        ...
        # সিকিউরিটি ফিক্স - পাথ স্যানিটাইজ
...
def install_pkg(server_id):
    try:
        ...
        if pkg_type == 'pip':
            ...
            SERVERS[server_id]['logs'].append("⚠️ apt-get may not work on Render free tier")
...
def update_config():
    try:
        ...
        stored_admin_pass = CONFIG.get('admin_password', DEFAULT_CONFIG['admin_password'])
        
        if hashed_input != stored_admin_pass:
...
if __name__ == "__main__":
    ...
    print("  Secret: shappno (Can change user password)")
    print("  User: shappno (Cannot change password)")
    print("=" * 50)
